...
export const AddContainerTab: React.FC<AddContainerTabProps> = ({ onAddContainer, onOpenQuickImport }) => {
  ...
  const [chaFirm, setChaFirm] = useState('Riddhi Siddhi CHA');
  ...
  const [recipientsList, setRecipientsList] = useState<FormRecipientItem[]>([
    ...
    },
    {
      ...
      recipientName: 'Ramesh — Ops Manager',
      isActive: true,
      isOwner: false,
    },
  ]);
  ...
  const calculatedLfdInfo = useMemo(() => {
    ...
    const lfd = calculateLFD(dischargeDate, selectedLine);
    const now = new Date();
  ...
  return (
    <div className="max-w-[800px] mx-auto space-y-6">
      ...
      {/* NEW: Quick Import from WhatsApp / PDF Banner */}
      <div className="bg-gradient-to-r from-orange-500/15 via-amber-500/10 to-orange-500/5 border border-orange-500/30 rounded-3xl p-5 sm:p-6 shadow-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-start sm:items-center gap-3.5">
          <div className="w-11 h-11 rounded-2xl bg-orange-500 text-white flex items-center justify-center font-bold shrink-0 shadow-lg shadow-orange-500/20">
            <Sparkles className="w-6 h-6" />
          </div>
          <div>
            <div className="flex items-center gap-2 mb-0.5">
              <span className="text-xs font-bold text-white uppercase tracking-wider">
                Have a Discharge List PDF?
              </span>
              <span className="px-2 py-0.5 bg-emerald-500/20 text-emerald-300 rounded-full text-[9px] font-mono-data font-black">
                AI Auto-Extract
              </span>
            </div>
            <p className="text-xs text-[var(--muted)] font-medium">
              Don&apos;t type container numbers manually. Forward or upload your Vessel Discharge List PDF to extract 10–50+ containers instantly.
            </p>
          </div>
        </div>
        ...
        <button
          type="button"
          onClick={() => onOpenQuickImport && onOpenQuickImport()}
          className="bg-gradient-to-r from-orange-500 to-amber-600 hover:from-orange-600 hover:to-amber-700 text-white font-extrabold px-5 py-2.5 rounded-2xl text-xs flex items-center gap-2 shadow-md shadow-orange-500/20 transition-all shrink-0 cursor-pointer w-full sm:w-auto justify-center"
        >
          <Upload className="w-4 h-4" />
          <span>⚡ Quick Import from PDF</span>
        </button>
      </div>
      ...
      <div className="bg-[var(--card)] border border-[var(--border)] rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4 pt-2">
            ...
            {calculatedLfdInfo && (
              <div className="bg-emerald-950/40 border border-emerald-500/40 p-4 rounded-2xl flex items-center justify-between text-xs">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-xl bg-emerald-500/20 flex items-center justify-center shrink-0">
                    <Calendar className="w-5 h-5 text-emerald-400" />
                  </div>
                  <div>
                    <span className="text-[var(--muted)] block text-[10px] font-mono-data uppercase font-bold">
                      Calculated Last Free Day (Demurrage LFD)
                    </span>
                    <span className="font-extrabold text-white text-base font-mono-data">
                ...
                <div className="text-right">
                  <span
                    className={`text-xs font-bold font-mono-data px-3 py-1.5 rounded-xl ${
                  ...
                  >
                    {calculatedLfdInfo.daysFromNow < 0
                      ? `Overdue (${Math.abs(calculatedLfdInfo.daysFromNow)}d)`
      ...
      {/* Bulk Upload Section Bento Card */}
      <div className="bg-[var(--card)] border border-[var(--border)] rounded-3xl overflow-hidden shadow-lg">
        <button
          ...
          className="w-full p-5 text-left flex items-center justify-between font-bold text-sm text-white hover:bg-[var(--card-hover)] transition-colors cursor-pointer"
        >
          <span className="flex items-center gap-2.5">
            <Upload className="w-5 h-5 text-orange-400" />
            <span>📤 Bulk Upload — Batch Add Multiple Containers</span>
          </span>
          {bulkUploadOpen ? (
        ...
        {bulkUploadOpen && (
          <div className="p-6 border-t border-[var(--border)] bg-[var(--panel)] space-y-4 text-xs">
            {/* Drop Zone */}
            <div className="border-2 border-dashed border-orange-500/30 hover:border-orange-500 bg-[var(--card)] p-8 rounded-2xl text-center space-y-3 transition-colors cursor-pointer group">
              <div className="w-12 h-12 rounded-2xl bg-orange-500/10 text-orange-400 flex items-center justify-center mx-auto group-hover:scale-110 transition-transform">
                <FileSpreadsheet className="w-6 h-6" />
              </div>
              <div>
                <p className="text-white font-bold text-sm">
                  Drop Excel (.xlsx) or CSV file here or click to upload
                </p>
                <p className="text-xs text-[var(--muted)] mt-1 font-medium">
                  Upload up to 500 containers at once for batch LFD calculation and multi-recipient alert scheduling.
                </p>
              </div>
            </div>

            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs text-[var(--muted)]">
              <span>
                Required columns: <strong className="text-white font-mono-data">Container Number | Shipping Line | Container Type | Discharge Date | Importer Name</strong>
              </span>
              <button
                onClick={() => alert('Downloading MAPS_Container_Bulk_Upload_Template.xlsx sample file.')}
                className="text-orange-400 hover:underline font-bold shrink-0 cursor-pointer"
              >
