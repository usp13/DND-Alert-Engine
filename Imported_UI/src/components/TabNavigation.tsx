...
import { LayoutDashboard, PlusCircle, TrendingUp, BookOpen, MessageSquare, Sparkles } from 'lucide-react';

export type TabType = 'dashboard' | 'auto-import' | 'add' | 'report' | 'rules' | 'whatsapp';
...
export const TabNavigation: React.FC<TabNavigationProps> = ({
  ...
  const tabs = [
    ...
    {
      id: 'auto-import' as TabType,
      label: 'Auto Import (PDF)',
      icon: Sparkles,
      highlightBadge: 'AI'
    },
    {
      ...
      <div className="max-w-[1240px] mx-auto flex items-center gap-2 overflow-x-auto scrollbar-none py-1">
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              <Icon className={`w-4 h-4 ${isActive ? 'text-white' : tab.id === 'auto-import' ? 'text-orange-400' : 'text-[var(--muted)]'}`} />
              <span>{tab.label}</span>
              {tab.badge !== undefined && (
