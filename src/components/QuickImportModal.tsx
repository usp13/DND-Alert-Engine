...
export const QuickImportModal: React.FC<QuickImportModalProps> = ({
  ...
  const handleProcessUploadedFile = async (file: File) => {
    ...
    setExtractionStepText(`Reading ${file.name}...`);

    try {
      const base64Data = await fileToBase64(file);
      setExtractionProgress(35);
      setExtractionStepText('Parsing PDF document structure & container streams...');

      const progressInterval = setInterval(() => {
        setExtractionProgress((prev) => (prev < 80 ? prev + 10 : prev));
      }, 300);
      ...
      if (!res.ok || !data.success) {
        throw new Error(data.error || 'Failed parsing PDF file.');
      }

      setExtractionProgress(90);
      setExtractionStepText('Matching container prefixes to shipping line rules & LFD schedules...');

      const rawContainers = data.containers || [];
      if (rawContainers.length === 0) {
        throw new Error('No valid 11-character container numbers (e.g. MSKU1234567) were detected in this document. You can add them manually in the editor.');
      }

      const inferredVessel = rawContainers[0]?.vessel_name || file.name.replace(/\.[^/.]+$/, '').replace(/_/g, ' ');
      ...
      setErrorMessage(err.message || 'Failed extracting container details from PDF.');
      setStep('upload');
    }
  };
  ...
  const startExtraction = (docToExtract: DischargeListDocument) => {
    ...
    const timer2 = setTimeout(() => {
      ...
      setExtractionStepText('Automated OCR parsing container numbers & ISO 6346 prefix classification...');
    }, 800);
  ...
  const handleConfirmImport = () => {
    ...
    onImportContainers(extractedRows, { vessel: vesselName, filename: customFile?.name || selectedDoc.filename });
    setStep('success');
    setTimeout(() => {
      onClose();
    }, 1800);
    ...
    <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
      <div className="bg-[#0d1117] border border-[var(--border)] rounded-3xl max-w-4xl w-full overflow-hidden shadow-2xl animate-fade-in flex flex-col max-h-[92vh]">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-heading font-black text-white text-base sm:text-lg">
                  PDF & Delivery Order Import Engine
                </h3>
              ...
              <p className="text-xs text-[var(--muted)] font-medium">
                Extract container numbers, shipping lines, discharge dates, and calculate LFDs directly from your PDF files.
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6">
          {errorMessage && (
            <div className="p-3.5 bg-red-950/50 border border-red-500/40 rounded-2xl flex items-center gap-2.5 text-xs text-red-300">
          ...
          {step === 'upload' && (
            <div className="space-y-6">
              ...
              <div
                ...
                onDrop={handleDrop}
                className={`border-2 border-dashed rounded-3xl p-8 text-center space-y-4 transition-all relative group cursor-pointer ${
                  isDragOver
                    ? 'border-orange-500 bg-orange-950/20'
                    : 'border-orange-500/30 hover:border-orange-500 bg-[#161b22]/70'
                }`}
              >
                <input
                  type="file"
                ...
                <div>
                  ...
                  <p className="text-xs text-[var(--muted)] mt-1 font-medium max-w-md mx-auto">
                    The engine extracts real container numbers, detects shipping lines via ISO prefixes, and computes free-time deadlines.
                  </p>
                </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-mono-data text-orange-400 font-extrabold uppercase tracking-wider flex items-center gap-1.5">
                    <FileText className="w-4 h-4" /> Or choose from sample Indian port discharge presets:
                  </span>
                  <span className="text-[11px] text-[var(--dim)] font-medium">Click to test instant parser</span>
                </div>
            ...
            <div className="py-12 px-4 text-center space-y-6 max-w-lg mx-auto">
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex justify-between text-[11px] font-mono-data text-[var(--muted)]">
                  <span>Target: {customFile?.name || selectedDoc.filename}</span>
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="px-2 py-0.5 bg-orange-500/20 text-orange-300 rounded-full font-mono-data text-[10px] font-bold">
                        {customFile?.name || selectedDoc.filename}
                      </span>
                    </div>
